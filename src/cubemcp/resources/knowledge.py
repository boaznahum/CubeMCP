...
def register_resources(mcp: FastMCP) -> None:
    ...
    def architecture() -> str:
        ...
        return _store.get("architecture", "No architecture info. Run 'cubemcp relearn' from the CLI first.")

    @mcp.resource("cubesolve://solvers")
    def solvers() -> str:
        """Available cube-solving algorithms and their characteristics."""
        return _store.get("solvers", "No solver info. Run 'cubemcp relearn' from the CLI first.")

    @mcp.resource("cubesolve://commands")
    def commands() -> str:
        """CLI commands, keyboard shortcuts, and GUI reference for CubeSolve."""
        return _store.get("cli", "No CLI info. Run 'cubemcp relearn' from the CLI first.")

    @mcp.resource("cubesolve://knowledge")
    def full_knowledge() -> str:
        """Full auto-generated knowledge base for CubeSolve."""
        return _store.get("project", "No knowledge. Run 'cubemcp relearn' from the CLI first.")
